...
namespace abstractplay
{
    ...
    public static partial class GuidGenerator
    {
        ...
        /*
         * I can't use the GUID structure because of how it messes with byte order.
        */
        public static byte[] GenerateSequentialGuid()
        {
            Guid g = GenerateTimeBasedGuid();
            List<byte> newba = new List<byte>();
            byte[] currba = g.ToByteArray();
            newba.Add(currba[7]);
            newba.Add(currba[6]);
            newba.Add(currba[5]);
            newba.Add(currba[4]);
            ...
            newba.Add(currba[14]);
            newba.Add(currba[15]);
            byte[] newg = newba.ToArray();
            return newg;
        }

        public static string HelperBAToString(byte[] ina)
        {
            return string.Concat(ina.Select(i => string.Format("{0:x2}", i)));
        }
        ...
        public static byte[] HelperStringToBA(string instr)
        {
            List<byte> outba = new List<byte>();
            foreach (string chunk in WholeChunks(instr, 2))
            {
                outba.Add(byte.Parse(chunk, System.Globalization.NumberStyles.HexNumber));
            }
            return outba.ToArray();
        }
    }
}
